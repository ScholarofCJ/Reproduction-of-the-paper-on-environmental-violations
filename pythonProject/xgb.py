...
Model = RandomizedSearchCV(estimator=XGBClassifier(random_state=123, use_label_encoder=False, eval_metric='logloss'),
...
#把最优参数保存导出#
with open(r'Model\Best_param_xgb.json', 'w') as f:
    json.dump(Model.best_params_, f)
model = Model.best_estimator_

# # 保存模型
with open(r'Model\xgboost_model.pkl', 'wb') as f:
    pickle.dump(model, f)
