...
param_distributions = {
    ...
    'n_components': [None] + list(range(1, min(x_train.shape[1], y_train.nunique())))
}
kfold = StratifiedKFold(n_splits=6, shuffle=True, random_state=666)
Model = RandomizedSearchCV(estimator=LinearDiscriminantAnalysis(),
                           param_distributions=param_distributions, scoring='roc_auc',
                           n_jobs=-1, cv=kfold, n_iter=100, random_state=666)

# 训练模型
Model.fit(x_train, y_train)
print(Model.best_params_)
#把最优参数保存导出#
with open(r'Model\Best_param_LDA.json', 'w') as f:
    json.dump(Model.best_params_, f)
model = Model.best_estimator_

# 保存模型
with open(r'Model\lda_model.pkl', 'wb') as f:
    pickle.dump(model, f)
