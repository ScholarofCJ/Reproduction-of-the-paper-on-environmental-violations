...
param_distributions = {
    ...
    'learning_rate': [0.001, 0.01, 0.05, 0.1, 0.2, 0.3, 0.5, 1.0],
    'algorithm': ['SAMME', 'SAMME.R']
}

kfold = StratifiedKFold(n_splits=6, shuffle=True, random_state=666)
Model = RandomizedSearchCV(estimator=AdaBoostClassifier(random_state=123),
...
#把最优参数保存导出#
with open(r'Model\Best_param_adaboost.json', 'w') as f:
    json.dump(Model.best_params_, f)
model = Model.best_estimator_

# 保存模型
with open(r'Model\adaboost_model.pkl', 'wb') as f:
    pickle.dump(model, f)
