...
# 开始对模型进行优化，调整参数
param_distributions = {
    'C': np.logspace(-2, 2, 7),  # C 从 0.01 到 100，7 个点
    'kernel': ['linear', 'poly', 'rbf', 'sigmoid'],  # 常用核函数
    'gamma': ['scale', 'auto'] + list(np.logspace(-3, 1, 5)),  # 对 rbf/poly/sigmoid 有效
    'degree': [2, 3, 4],  # 仅对 poly 核有效
    'coef0': [0.0, 0.1, 0.5, 1.0]  # 对 poly 和 sigmoid 核有效
}
kfold = StratifiedKFold(n_splits=6, shuffle=True, random_state=666)
Model = RandomizedSearchCV(estimator=SVC(probability=True, random_state=123),
                           param_distributions=param_distributions, scoring='roc_auc',
                           n_jobs=-1, cv=kfold, n_iter=5, random_state=123)

# 训练模型
Model.fit(x_train, y_train)
print(Model.best_params_)
#把最优参数保存导出#
with open(r'Model\Best_param_svm.json', 'w') as f:
    json.dump(Model.best_params_, f)
model = Model.best_estimator_

# 保存模型
with open(r'Model\svm_model.pkl', 'wb') as f:
    pickle.dump(model, f)
# 预测
